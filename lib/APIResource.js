...
class APIResource {
    ...
    constructor(obj, client) {
        this._client = client || rentshare.default_client;
        this._set_obj(obj);
        return new Proxy(this, {
            get: function(target, key, receiver) {
                if (key in target._obj)
...
APIResource.request = function(method, params) {
    ...
    var client = params.client || rentshare.default_client
    if (params.id)
        path += '/' + params.id
    ...
    return new Promise(function(resolve, reject) {
        try {
            request[method]({
                ...
                if (!object_type)
                    throw new exc.InvalidResponse('Response missing "object" attribute')

                if (status_code != 200) {
                    if (object_type != 'error')
                        throw InvalidResponse('Expected error object')
                    for (var e in exc) {
                        e = exc[e];
                        if (e.status_code != status_code)
                            continue;
                        if (e.error_type && e.error_type != obj.error_type)
                            continue;
                        throw new e(obj.error_description)
                    }
                    throw new exc.APIException(obj.error_description)
                }
...
APIResource.delete_all = function(objects) {
    var deletes = objects.map(function(o) {
        return o.id
    }).join('|')
    return this.request('delete', {
        params: {
